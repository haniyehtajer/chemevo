...
print("n_gals in each alpha ia = ", n_gals)
print("n_gals total = ", n_gals * len(Upsilons))
...
def get_sampled_gals(params, alpha_cc=alpha_cc_def, alpha_Ia=alpha_ia_def, g_ratio=gcc_ratio_def, gcc=gcc_Mn_def, Upsilon = Upsilon_def):
    gals_list = []
...
for Upsilon_count in range(len(Upsilons)):
    Upsilon_this_model = Upsilons[Upsilon_count]
    gals_list = []
    # Change all params to sampled params if you only want a few random models
    gals_list = get_sampled_gals(all_params, Upsilon=Upsilon_this_model)
    ...
    df_endpoints.to_csv(f"models/Upsilon_{Upsilon_count}.csv", index=False)
