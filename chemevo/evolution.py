...
class Galaxy:
    def __init__(self, t_array, m_g_array, DTD_func = "exp",
                 ...
                 R_Ia_Mn = 1.97, alpha_cc_Mn = 0.17, alpha_Ia_Mn = 0.30):
        """
    ...
    def compute_z_X(self, element = "Mn", cc_ref_element = "O"):
        ...
        f_Ia_Fe = 1 - f_cc_Fe
        f_Ia_X = 1 - f_cc_X
        K_X_Ia = self.K_Fe_Ia_arr * (SolarX/self.SolarFe) * (f_Ia_X/f_Ia_Fe) * 10**(alpha_Ia_X * self.O_H)

        if cc_ref_element == "O":
            m_cc_ref_arr = self.m_O_cc_arr
