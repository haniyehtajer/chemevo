...
parser = argparse.ArgumentParser(description="Run iterative fitting with custom initial parameters.")
parser.add_argument("--alpha_cc", type=float, default=0.80, help="Initial alpha_cc")
parser.add_argument("--alpha_Ia", type=float, default=0.40, help="Initial alpha_Ia")
parser.add_argument("--g_cc", type=float, default=0.40, help="Initial g_cc")
parser.add_argument("--g_ratio", type=float, default=1.90, help="Initial g_ratio")
# ADD THIS LINE:
parser.add_argument("--save_dir", type=str, default="models_output", help="Folder to save and load models")
...
def optimize_for_param(param_name, file_pattern, save_dir):
    ...
    for file in model_files:
        ...
        param_val = model_df[param_name].iloc[0]
        if param_val <= 0:
            continue
...
num_iters = 10 # Increased so we can see the grid size change at iteration 3 and 6

# 1. Store your initial parameters in a dictionary
current_params = {
    "alpha_cc": args.alpha_cc,
    "alpha_Ia": args.alpha_Ia,
    "g_cc": args.g_cc,
...
for i in range(num_iters):
    for param in params_to_optimize:
        ...
        best_val = optimize_for_param(param_name=param, file_pattern=f"{args.save_dir}/{param}_iter{i}_*.csv", save_dir=args.save_dir)
