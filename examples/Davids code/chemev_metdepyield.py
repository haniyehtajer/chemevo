...
chemev.py yieldindex case outfile [case-dependent parameters]
    yieldindex : yield is ~ (O/Osolar)**yieldindex
...
Version chemev_metdepyield evolves one element with an oxygen-dependent yield
"""
...
dtd='plaw'	        # form for SNIa DTD: exp or plaw
tauIa=1.5		# e-folding time for exponential DTD
tdmin=0.15		# minimum delay time

mocc=0.015		# IMF-averaged CCSN oxygen yield
mfecc=0.0012		# IMF-averaged CCSN iron yield
mfeIa=0.0017		# SNIa iron yield over time interval tmax
r=0.4			# recyling fraction

SolarO=0.0056		# solar oxygen abundance by mass
SolarFe=0.0012		# solar iron abundance by mass

dt=0.010		# timestep for integration
dtout=0.020		# timestep for outputs
tmax=12.5		# maximum time
mdotfid=1.0		# fiducial SFR in Msun/year

#yieldindex=0.25		# yield is ~ (O/Osolar)**yieldindex

if (len(sys.argv)>2):
    yieldindex=float(sys.argv[1])
    case=sys.argv[2]
else:
    case='options'
...
if (case=='Simple'):
    ...
    taustar0=float(sys.argv[4])
    eta0=float(sys.argv[5])
    tausfh=float(sys.argv[6])
    sfhmode=sys.argv[7]
elif (case=='TwoPhase'):
    if (len(sys.argv)!=11):
        sys.exit("Args: taustar1 eta1 tausfh1 taustar2 eta2 tausfh2 tc")
    taustar1=float(sys.argv[4])
    eta1=float(sys.argv[5])
    tausfh1=float(sys.argv[6])
    taustar2=float(sys.argv[7])
    eta2=float(sys.argv[8])
    tausfh2=float(sys.argv[9])
    tc=float(sys.argv[10])
    sfhmode='exp'
elif (case=='PLSchmidt'):
    if (len(sys.argv)!=8):
        sys.exit("Args: taustar eta tausfh sfhmode")
    ...
    etaburst=float(sys.argv[11])
    burstmode=sys.argv[12]
elif (case=='GasBolus'):
    if (len(sys.argv)!=10):
      sys.exit("Args: taustar eta tausfh sfhmode tc factor")
    taustar0=float(sys.argv[4])
    eta0=float(sys.argv[5])
    tausfh=float(sys.argv[6])
    sfhmode=sys.argv[7]
    ...
    taustar0=float(sys.argv[4])
    eta0=float(sys.argv[5])
    mdotinf0=float(sys.argv[6])
    mdotstar0=float(sys.argv[7])
    infall=True
    ...
    mdotstar0=float(sys.argv[7])
    t1=float(sys.argv[8])
    dt1=float(sys.argv[9])
    dt2=float(sys.argv[10])
    factor=float(sys.argv[11])
    burstmode=sys.argv[12]
    infall=True
else:
...
fX=		np.zeros(nstep+1,dtype=float)	# ISM mass fraction of element X
...
for i in range(1,nstep+1):
    ...
    miron += dt*(mfecc*mgas[i-1]-(1+eta[i-1]-r)*miron)/taustar[i-1] + \
             dt*mfeIa*mdotstarIa
    Xyield=mocc*((foxy[i-1]/SolarO+1.e-6)**yieldindex)
    mX += dt*(Xyield*mgas[i-1]-(1+eta[i-1]-r)*mX)/taustar[i-1]
    foxy[i]=moxy/mgas[i]
...
np.savetxt(outfile,
           np.transpose([t[n::n],mdotstar[n::n],firon[n::n],ofe[n::n],Xo[n::n],
	                mgas[n::n],mdotinf[n::n],eta[n::n],taustar[n::n]]),
           '%6.3f %6.3f %7.3f %7.3f %7.3f %8.3e %6.3f %6.3f %6.3f')
