### FOR VICE TESTING ###
### Takes total Z as met, not just mg ###
...
class Galaxy:
    def __init__(self, t_array, m_g_array, DTD_func = "exp",
        ...
        # Yields
        if yields_ref == "W2024":
            self.m_O_cc_arr   = self._make_array(71.3e-4) * self.Upsilon
            self.m_Mg_cc_arr = self._make_array(6.52e-4) * self.Upsilon
            self.m_Fe_cc_arr  = self._make_array(4.73e-4) * self.Upsilon
            self.m_Fe_Ia_arr  = self._make_array(7.7e-4) * self.Upsilon
        ...
        elif yields_ref == "W2024,moreFe":
            self.m_O_cc_arr   = self._make_array(71.3e-4) * self.Upsilon
            self.m_Mg_cc_arr = self._make_array(6.52e-4) * self.Upsilon
            self.m_Fe_cc_arr  = self._make_array(4.73e-4) * self.Upsilon
            self.m_Fe_Ia_arr  = self._make_array(7.7e-4) * 1.1 * self.Upsilon
            self.K_Fe_Ia_arr = self._make_array(0.77)
            self.DTD_R0 = 1.3e-3/tau_Ia

        elif yields_ref == "W2024,changed-plateau":
            self.m_O_cc_arr   = self._make_array(71.3e-4) * self.Upsilon
            self.m_Mg_cc_arr = self._make_array(6.52e-4) * self.Upsilon
            self.m_Fe_cc_arr  = self._make_array(4.73e-4) * self.Upsilon * 10**(-0.1)
            self.m_Fe_Ia_arr  = self._make_array(7.7e-4) * 1.1 * self.Upsilon ** 10**(0.25)
            self.K_Fe_Ia_arr = self._make_array(0.77)
            self.DTD_R0 = 1.3e-3/tau_Ia

        elif yields_ref == "W2024,moreFe30":
            self.m_O_cc_arr   = self._make_array(71.3e-4) * self.Upsilon
            self.m_Mg_cc_arr = self._make_array(6.52e-4) * self.Upsilon
            self.m_Fe_cc_arr  = self._make_array(4.73e-4) * self.Upsilon
            self.m_Fe_Ia_arr  = self._make_array(7.7e-4) * 1.3 * self.Upsilon
            self.K_Fe_Ia_arr = self._make_array(0.77)
            self.DTD_R0 = 1.3e-3/tau_Ia
        ...
        if yields_ref == "sanders-test":
            self.m_O_cc_arr   = self._make_array(0.011728) * self.Upsilon
            self.m_Mg_cc_arr = self._make_array(6.52e-4) * self.Upsilon
            self.m_Fe_cc_arr  = self._make_array(4.73e-4) * self.Upsilon
            self.m_Fe_Ia_arr  = self._make_array(7.7e-4) * self.Upsilon
            self.K_Fe_Ia_arr = self._make_array(0.77)
    ...
    def compute_z_X(self, element="Mn"):
        ...
        for i in range(1, self.n_steps):
            # 1. Calculate z_X from the previous step
            z_X_prev = m_X[i-1] / self.m_g_array[i-1]
            
            # 2. Update Z_total for the current step
            current_Z_total = Z_base[i-1] + z_X_prev
            z_ratio = current_Z_total / Z_total_solar
            
            # 3. Calculate yields using the new Z_total
            current_m_X_cc = self.Upsilon * self.g_cc_Mn * SolarX * (z_ratio ** alpha_cc_X)
            current_m_X_Ia = self.Upsilon * g_Ia_Mn * SolarX * (z_ratio ** alpha_Ia_X)
            
            # 4. Integrate the mass of element X
            m_X[i] = m_X[i-1] + self.dt * ( 
                (current_m_X_cc * self.m_g_array[i-1] / self.tau_star_arr[i-1]) + 
                (current_m_X_Ia * mdotstar_Ia[i-1]) - 
    ...
    def compute_z_X_CC(self, element = "Mn"):
        ...
        y_X_cc = self.Upsilon * self.g_cc_Mn * SolarX * 10**(alpha_cc_X * self.Mg_H)
      
        m_X_cc = np.zeros(self.n_steps)
        for i in range(1, self.n_steps):
            m_X_cc[i] = m_X_cc[i-1] + self.dt*( (y_X_cc[i-1] * self.m_g_array[i-1] / self.tau_star_arr[i-1]) - m_X_cc[i-1]/self.tau_dep_arr[i-1] )
    ...
    def compute_z_X_Ia(self, element = "Mn"):
        ...
        y_X_Ia = self.Upsilon * g_Ia_Mn * SolarX * 10**(alpha_Ia_X * self.Mg_H)

        mdotstar_Ia = self.compute_mdotstar_Ia()
        
        m_X_Ia = np.zeros(self.n_steps)
        for i in range(1, self.n_steps):
            m_X_Ia[i] = m_X_Ia[i-1] + self.dt*( y_X_Ia[i-1] * mdotstar_Ia[i-1] - m_X_Ia[i-1]/self.tau_dep_arr[i-1] )
    ...
    def compute_z_T(self, alpha_cc_T=1, ): # A test element that is metallicity dependant but is only produced through CCSNe
        ...
        #m_T_cc = self.Upsilon * self.g_cc_Mn * SolarX * 10**(alpha_cc_T * self.Mg_H)
        #m_T_cc = self.Upsilon * self.g_cc_Mn * 0.00445625 * 10**(alpha_cc_T * self.Mg_H)
        #m_T_cc = self.Upsilon * self.g_cc_Mn * 10**(alpha_cc_T * self.Mg_H)
        m_T_cc = self.Upsilon * self.g_cc_Mn * 2. * self.SolarO * 10**(alpha_cc_T * self.Mg_H)

        m_T = np.zeros(self.n_steps)
        for i in range(1, self.n_steps):
            m_T[i] = m_T[i-1] + self.dt*( (m_T_cc[i-1] * self.m_g_array[i-1] / self.tau_star_arr[i-1])
                                            - m_T[i-1]/self.tau_dep_arr[i-1] )

        z_T = m_T/self.m_g_array
        return z_T
