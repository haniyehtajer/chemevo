...
for g_ratio_count in range(len(g_ratios)):
    g_ratio_this_model = g_ratios[g_ratio_count]
    gals_list = []
    # Change all params to sampled params if you only want a few random models
    gals_list = get_sampled_gals(all_params, g_ratio=g_ratio_this_model)
    ...
    df_endpoints.to_csv(f"models/g_ratio_iter2_{g_ratio_count}.csv", index=False)
