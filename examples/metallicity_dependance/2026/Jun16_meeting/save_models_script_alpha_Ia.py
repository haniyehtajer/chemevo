...
for alphaia_count in range(len(alpha_Ias)):
    alpha_Ia_this_model = alpha_Ias[alphaia_count]
    gals_list = []
    # Change all params to sampled params if you only want a few random models
    gals_list = get_sampled_gals(all_params, alpha_Ia=alpha_Ia_this_model)
    ...
    df_endpoints.to_csv(f"models/alpha_Ia_iter2_{alphaia_count}.csv", index=False)
