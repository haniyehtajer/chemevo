...
print("n_gals total = ", n_gals * len(g_ccs))
...
for g_cc_count in range(len(g_ccs)):
    g_cc_this_model = g_ccs[g_cc_count]
    gals_list = []
    # Change all params to sampled params if you only want a few random models
    gals_list = get_sampled_gals(all_params, g_cc=g_cc_this_model)
    ...
    df_endpoints.to_csv(f"models/g_cc_iter2_{g_cc_count}.csv", index=False)
